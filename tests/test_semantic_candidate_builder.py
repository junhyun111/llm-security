...
"""
    ...
    assert candidates[0].feature_schema_version == "semantic-cwe-v3"
...
"""
    ...
    assert helper.related_functions[0].relation == "caller"
    assert helper.related_functions[0].function == "caller"
    assert caller.related_functions[0].relation == "callee"
    assert caller.related_functions[0].function == "helper"
    assert [candidate.candidate_id for candidate in first] == [
        candidate.candidate_id for candidate in second
    ]

    context = ContextBuilder().build(helper, ExpertFamily.MEMORY_SAFETY)
    assert "[caller]" in context.related_functions_text
    assert "function=caller" in context.related_functions_text
    assert "helper" in context.code_slice
    assert "EV-" in context.evidence_graph_text
