...
- `evaluation.ipynb`: run a deterministic 500-case Test diagnostic by default,
  report micro/per-Expert/macro stage metrics, and evaluate the complete frozen
  Test split of 8,337 scenarios only after `DIAGNOSTIC_CASE_LIMIT=0` is set.
  Batched patch verification is separately disabled by default.
...
Candidate selection has two explicit contracts. Train/Dev outcome construction
uses `training_matrix`, which may use ground truth to retain positives and hard
negatives. Test uses `deployment_top_k`, which reads only learned ranker scores
and deterministic tie-breakers. Planning writes the exact selected IDs plus
case/cache/manifest SHA-256 values; collection and final evaluation reject a
different policy, cache, case file, Top-K value, or manifest.
...
The CLI and web factory load the same Candidate Ranker used by the evaluation
notebook. Configure `CANDIDATE_RANKER_PATH`, keep
`CANDIDATE_RANKER_REQUIRED=true`, set `MAX_CANDIDATES=4`, and point
`WEB_ROUTER_ARTIFACT` at the Router generated by the current `train.ipynb`.
Incompatible or missing ranker/router artifacts fail closed instead of falling
back to a hand-written candidate score. Copy the calibrated per-Expert values
from `training_report.json` into the matching `MINIMUM_CONFIDENCE_*` entries in
`.env`.
...
work/router_evaluation_diagnostic_500/
results/router_evaluation_diagnostic_500/evaluation_*.json
```
