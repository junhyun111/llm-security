...
class LoggedLLMClient:
    """Pass-through client that records usage without requiring a budget flag."""

    def __init__(self, client, ledger_path: str | Path) -> None:
        self.client = client
        self.ledger_path = Path(ledger_path)
        self.requests = 0
        self.actual_usd = 0.0
    ...
    def complete(self, *, model, messages, response_schema, metadata=None):
        request_number = self.requests + 1
        task = str((metadata or {}).get("task", "unknown"))
        try:
            response = self.client.complete(
                model=model,
            ...
            raise
        self.requests += 1
        self.actual_usd += float(response.usage.cost)
        append_jsonl(
            self.ledger_path,
            {
                "timestamp_utc": datetime.now(timezone.utc).isoformat(),
